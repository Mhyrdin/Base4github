    ...
    static class Visualizers
    {
        ...
        static void DrawGizmos(WaterRenderer target, GizmoType type)
        {
            ...
            if (target._ProxyPlane != null && target._ProxyPlane.activeSelf != target._ShowWaterProxyPlane)
            {
                ...
                // Scene view doesn't automatically refresh which makes the option confusing, so force it
        ...
        static void DrawGizmos(LodInput target, GizmoType type)
        {
            ...
            if (target._DrawBounds)
            {
                ...
                if (rect != Rect.zero)
                {
                    ...
                    var height = water ? water.SeaLevel + 0.001f : target.Transform.position.y;
        ...
        static void DrawGizmos(LevelLodInput target, GizmoType type)
        {
            ...
            if (target.Mode == LodInputMode.Area) return;
        ...
        [DrawGizmo(GizmoType.Selected)]
        static void DrawAreaGizmos(Area target, GizmoType type)
        {
            ...
            if (!target._HasLevel && water != null) position.y = water.SeaLevel + 0.001f;
            ...
            Gizmos.DrawWireCube(Vector3.zero, Vector3.one.XNZ());
        }
        ...
        static void DrawWatertightHullGizmos(WatertightHull target, GizmoType type)
        {
            ...
            if (target._Debug._DrawBounds)
            {
                ...
                {
                    var water = WaterRenderer.Instance;
                    var height = water ? water.SeaLevel : transform.position.y;
                    Gizmos.color = Color.magenta;
                    Gizmos.DrawWireCube
                    (
                        new(rect.center.x, height, rect.center.y),
        ...
        static void DrawGizmos(DepthProbe target, GizmoType type)
        {
            ...
            if (target.Type == DepthProbeMode.RealTime)
            {
                ...
                position.y = Mathf.LerpUnclamped(target._CaptureRange.x, target._CaptureRange.y, 0.5f);
                ...
                if (target._FillHolesCaptureHeight > 0f)
                {
                    Gizmos.color = Color.magenta;
        ...
        static void DrawGizmos(WaterBody target, GizmoType type)
        {
            ...
            var size = target.AABB.size;
            if (!target.Vertical) size.y = 1f;
        ...
        {
            var xPoints = new Vector3[(Mathf.FloorToInt(size.x / 2f) + 1) * 2];
            var zPoints = new Vector3[(Mathf.FloorToInt(size.y / 2f) + 1) * 2];
