...
namespace WaveHarmonic.Crest
{
    ...
    abstract class QueryBase : IQueryable
    {
        ...
        /// <summary>
        /// Since query results return asynchronously and may not return at all (in theory), we keep a ring buffer
            ...
            public void AcquireNew()
            {
...

                {
                    ...
                    if (newSegmentAcquire == _SegmentRelease)
                    {
                        ...
                        // can happen if the Scene and Game view are not visible, in which case async readbacks don't get processed
                ...
                // queries are often made from FixedUpdate(), and at high frame rates this may not be called, which would mean
        ...
        protected bool UpdateQueryPoints(int ownerHash, float minSpatialLength, Vector3[] queryPoints, Vector3[] queryNormals)
        {
            ...
            if (!segmentRetrieved)
            {
                ...
                {
                    Debug.LogError("Crest: Too many GUIDs registered with CollProviderCompute. Increase k_MaximumGUIDs.");
                    return false;
                }
        ...
        public void SendReadBack(WaterRenderer water)
        {
            ...
            if (_SegmentRegistrarRingBuffer.Current._QueryCount > 0)
            {
                ...
                // Previously, subtracted delta time which it introduced jitters. Smooth delta time
                // helps, but I am not convinced it is needed.
        ...
        void DataArrived(AsyncGPUReadbackRequest req)
        {
            ...
            // Multiple readbacks can complete in one frame. On subsequent calls, this will be
            // negative since all completed readbacks have been processed by the first call.
            if (lastDoneIndex < 0)
            {
                return;
            }
